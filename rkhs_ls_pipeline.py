...
import kernel_evaluation as KE


def recolor(*, dest, dataset_src: str, make_k, sample_function, gamma):
    ...
    g = KE.make_g(norm_im_g)

    # now we pick our mask a.k.a. D \subsetof Omega
    mask_arr = sample_function(norm_im_rgb)
    ...
    D, F_D, _ = KE.sample_rgb_on_mask(norm_im_rgb, mask_arr)

    # compute K_D and K_cD
    H, W, _ = norm_im_rgb.shape
    K_D = KE.kernel_gram_matrix(k, D)  # (m,m)
    K_cD = KE.kernel_cross_matrix_Omega_D(k, D, H, W)

    # solve for A
    A = KE.solve_for_A(K_D, F_D, gamma=gamma, jitter=1e-10)

    # reconstruct F_gamma
    F_gamma_normalized = KE.reconstruct_image_via_KcD(K_cD, A, H, W)
